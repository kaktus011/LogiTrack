...
namespace LogiTrack.Tests
{
    ...
    public class OfferServiceTests
    {
        ...
        [Test]
        public void TestClientCompanyExists()
        {
            var clientCompany = dbContext.ClientCompanies
                .FirstOrDefault(c => c.Name == "Client Company 1");

            Assert.IsNotNull(clientCompany);
            Assert.AreEqual(clientCompany.Name, "Client Company 1");
            Assert.AreEqual(clientCompany.ContactPerson, "John Doe");
        }
        ...
        public void TestDriverExists()
        {
            var driver = dbContext.Drivers
                .FirstOrDefault(d => d.Name == "John Doe");

            Assert.IsNotNull(driver);
            Assert.AreEqual(driver.LicenseNumber, "D12345678");
            Assert.AreEqual(driver.Salary, 3000.0m);
        }
        ...
        public void TestVehicleExists()
        {
            var vehicle = dbContext.Vehicles
                .FirstOrDefault(v => v.RegistrationNumber == "ABC123");

            Assert.IsNotNull(vehicle);
            Assert.AreEqual(vehicle.VehicleType, "Truck");
            Assert.AreEqual(vehicle.MaxWeightCapacity, 18000);
        }
        ...
        public void TestRequestExists()
        {
            var request = dbContext.Requests
                .FirstOrDefault(r => r.RerefenceNumber == "R0001");

            Assert.IsNotNull(request);
            Assert.AreEqual(request.CargoType, "Standard");
            Assert.AreEqual(request.Status, "Pending");
        }

        [Test]
        public void TestOfferExists()
        {
            var offer = dbContext.Offers
                .FirstOrDefault(o => o.OfferNumber == "OFFER0001");

            Assert.IsNotNull(offer);
            Assert.AreEqual(offer.FinalPrice, 1200.0m);
            Assert.AreEqual(offer.OfferStatus, "Pending");
        }
        ...
        public void TestInvoiceExists()
        {
            var invoice = dbContext.Invoices
                .FirstOrDefault(i => i.InvoiceNumber == "INV001");

            Assert.IsNotNull(invoice);
            Assert.AreEqual(invoice.PaidOnTime, true);
            Assert.AreEqual(invoice.Status, "Paid");
        }

        [Test]
        public void TestCashRegisterExists()
        {
            var cashRegister = dbContext.CashRegisters
                .FirstOrDefault(c => c.Description == "Fuel Expense");

            Assert.IsNotNull(cashRegister);
            Assert.AreEqual(cashRegister.Amount, 100.00m);
            Assert.AreEqual(cashRegister.Type, "Vehicle Expenses");
        }

        [Test]
        public void TestDeliveryExists()
        {
            var delivery = dbContext.Deliveries
                .FirstOrDefault(d => d.ReferenceNumber == "DEL0001");

            Assert.IsNotNull(delivery);
            Assert.AreEqual(delivery.Status, "Delivered");
            Assert.AreEqual(delivery.TotalExpenses, 500.00m);
        }
