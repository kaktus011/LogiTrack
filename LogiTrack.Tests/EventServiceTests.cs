...
namespace LogiTrack.Tests
{
    ...
    public class EventServiceTests
    {
        ...
        public async Task GetUserEventsAsync_ShouldReturnUserEvents()
        {
            ...
            var result = await eventService.GetUserEventsAsync(username);

            Assert.IsNotNull(result);
            Assert.AreEqual(1, result.Count); 
            Assert.AreEqual("Monthly Payment Due", result[0].Title);
            Assert.AreEqual("Paid", result[0].Type);
            Assert.AreEqual(DateTime.Now.AddMonths(-1).ToString("dd-MM-yyyy"), result[0].Date.ToString("dd-MM-yyyy"));
        }
